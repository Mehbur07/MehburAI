...
- 🔔 **Güncelleme Uyarısı:** MehburAI açılınca (ve açık kaldıkça 6 saatte bir) GitHub'daki en yeni sürüm numarasına bakar; bu bilgisayardakinden yeniyse pencerenin altında *"Uyarı: MehburAI'a güncelleme geldi. Eğer yeni sürümü indirmek istiyorsanız bu adresten indirebilirsiniz: <GitHub bağlantısı>"* şeridi çıkar (bağlantı tıklanabilir, ✕ ile kapanır). Önce GitHub *Releases*'in son etiketine (`v1.5` gibi), yayın yoksa depodaki `config.py > APP_VERSION`'a bakılır (`updater.py`, adresler `config.GITHUB_REPO` / `UPDATE_PAGE_URL`). **Not:** GitHub depo *gizli (private)* iken dışarıdan hiçbir şey görünmez — denetim sessizce atlanır; başka bilgisayarlarda çalışması için sürüm bilgisinin/Release'in herkese açık bir yerde durması gerekir.
